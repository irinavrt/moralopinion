...
table_for_print <- arg_connection %>%
  ...
  mutate(question = ifelse(lib_position, str_c(question, "*"), question),
         question = ifelse(coders == 2, str_c(question, "\\textdagger"), question),
         question = str_replace(question, "OWNER DECIDES and CAN'T DISCRIMINATE", tolower("OWNER DECIDES and CAN'T DISCRIMINATE"))) %>%
...
than conservatives in the GSS. A dagger (\\textdagger) marks the 50 items that both coders agreed upon. Opinion
...
minus the same average for the opposing position.") %>%
  kable_styling(full_width = FALSE,
                ...
                repeat_header_text = "\\textbf{The 74 GSS items used in the study.} \\textit{(continued)}",
