...
```{r sfig5, fig.width = 4, fig.height = 3, fig.cap = "**Mean correlation between harm-fairness connection advantage and rate of change as a function of sample size of argument measures.** The average correlation based on 200 random draws of n responses. Small sample sizes lead to an underestimation of the main effect, but not its precision. Sample sizes above 40 are sufficient to get reliable estimates of the harm-fairness connection advantage. Note that the sample used has on average 176 responses per issue."}
...
by_n <- tibble(n_samp = rep(c(1, 2, 3, 5, 10, 20, 40, 60, 80, 100, 120, 150), 
                            each = 200)) 
by_n <- by_n %>% 
  mutate(cor = map(n_samp, draw_cor_for_selected_n_orig_response))

by_n %>% 
  unnest(cor) %>% 
  group_by(n_samp) %>% 
  summarise(mean = mean(cor)) %>%
  ggplot(aes(n_samp, mean)) +
  geom_line() +
  theme_classic(base_size = 11) %>% 
  labs(x = "n")
